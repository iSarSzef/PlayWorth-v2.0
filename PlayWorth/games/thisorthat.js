...
function SurrenderChosen() {
    ...
    // Update saldo: add back the current winnings
    if (typeof userSaldo !== "undefined") {
        const newSaldo = userSaldo + moneywon;
        userSaldo = newSaldo; // Update local variable immediately
        updateSaldo(newSaldo);
    }
}
...
function ShowCard(r){
    ...
    img_url = "/PlayWorth/others/Cards/" + cards[r] + ".png"
...
function win(){
    ...
    console.log(result.style.height)
    console.log(result.style.backgroundColor)
    // Update saldo: add winnings to balance
    if (typeof userSaldo !== "undefined") {
        const newSaldo = userSaldo + moneywon;
        userSaldo = newSaldo;
        updateSaldo(newSaldo);
...
function gameEnds() {
    ...
    // Remove all shown cards and all .card elements from #parent
    const parent = document.getElementById("parent");
    while (parent.firstChild) {
        parent.removeChild(parent.firstChild);
    }

    // Create and append a fresh main card
    const newCard = document.createElement("div");
    newCard.className = "card";
    newCard.id = "current_card";
    newCard.style.transform = "rotateY(0deg)";
    newCard.style.backgroundImage = 'url("/PlayWorth/others/Cards/back.png")'; // lowercase 'back.png' for consistency with CSS
